...
var makeLines = function(n, lines, prevScore){
  ...
  var isHorizontal = flip(0.5);
  
  var x1 = randomInteger(50);
  var y1 = randomInteger(50);
  var x2 = isHorizontal ? randomInteger(50) : x1;
  var y2 = isHorizontal ? y1 : randomInteger(50);
  ...
  var newScore = -targetImage.distance(generatedImage)/500; // Increase to 10000 to see more diverse samples
...
ParticleFilter(
  function(){
    // vary numLines to see how the particle filter evolves
    var numLines = 4;
    var lines = makeLines(numLines, [], 0);
...
// examine 3 of the samples
repeat(3, function() {
  var image = Draw(50, 50, true);
  var lines = sample(finalImgSampler);
...
var drawLines = function(drawObj, lines){
  var line = lines[0];
  drawObj.line(line[0], line[1], line[2], line[3], line[4], line[5], line[6], true);
  if (lines.length > 1) {
    drawLines(drawObj, lines.slice(1));
  }  else {
    drawObj.redraw()
