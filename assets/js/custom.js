...
// added delay parameter to make final computer vision example faster
DrawObject.prototype.line = function(x1, y1, x2, y2, strokeWidth, opacity, color, delay){
...
DrawObject.prototype.distanceF = function(f, cmpDrawObject){
  ...
  // hack to make final computer vision example faster by store imgData
  // (presumably of target image)
  if (!this.imgData) {
    this.imgData = this.toArray();
  }
  //var thisImgData = this.toArray();
  var cmpImgData = cmpDrawObject.toArray();
  return f(this.imgData, cmpImgData);
};
